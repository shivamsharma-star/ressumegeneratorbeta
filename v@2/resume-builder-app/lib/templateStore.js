...
import { assertTemplateIsValid } from './templateValidation';
...
function saveTemplateInternal({ name, buffer }) {
  ...
  // Fail fast, with a clear reason, instead of only discovering a broken
  // template the moment a user tries to generate their resume from it.
  assertTemplateIsValid(sanitized);
